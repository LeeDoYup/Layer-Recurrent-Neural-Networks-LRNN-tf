		...
		with tf.variable_scope('CONV1') as scope:
			...
			self.layers['relu_conv1'] =  tf.nn.relu(self.layers['conv1']+self.paras['b1'], name=scope.name)
                ...
                with tf.variable_scope('pool1') as scope:
                        self.layers['pool1'] = tf.nn.max_pool(self.layers['CNN1'],ksize=[1,2,2,1],strides=[1,2,2,1],padding='SAME')

                ##########################                  pool 1               ######################################


                with tf.variable_scope('CNN2') as scope:
                        self.layers['CNN2'] = CNN_module(self.layers['pool1'],'Concatnate',128,128)


                ########################################################################################################

                with tf.variable_scope('pool2') as scope:
                        self.layers['pool2'] = tf.nn.max_pool(self.layers['CNN2'],ksize=[1,2,2,1],strides=[1,2,2,1],padding='SAME')

                ##########################                  pool 2               ######################################

 
                with tf.variable_scope('CNN3') as scope:
                        self.layers['CNN3'] = CNN_module(self.layers['pool2'],'Concatnate',256,256)
  

               #########################################################################################################
 
                with tf.variable_scope('pool3') as scope:
                        self.layers['pool3'] = tf.nn.max_pool(self.layers['CNN3'],ksize=[1,8,8,1],strides=[1,8,8,1],padding='SAME')

                with tf.variable_scope('dropout') as scope:
                        self.layers['dropout'] = tf.nn.dropout(self.layers['pool3'],0.5)

                with tf.variable_scope('softmax') as scope:
                        self.paras['W_out'] = weight_variable('W_out',[512,10])
                        self.paras['b_out'] = bias_variable('b_out',[10])
                        self.layers['output'] = full_connected(tf.reshape(self.layers['dropout'],[-1,512]),self.paras['W_out'],self.paras['b_out'],activation ='Softmax')
