...
class Model(object):
	def __init__(self, sess,x_train,y_train):
		...
		self.min_valid_loss = 9999.999
                ...
                self.train_log = open("train_loss.txt",'w')
                self.validation_log = open("validation_loss.txt",'w')             
	...
	def _inference(self):
		...
		with tf.variable_scope('CONV1') as scope:
			self.paras['W1'] = weight_variable('W1',[3,3,3,16],initializer = 'Xavier')
			self.paras['b1'] = bias_variable('b1',[16])
			...
			self.layers['relu_conv1'] =  tf.nn.relu(self.layers['conv1']+self.paras['b1'], name=scope.name)
                ...
                ##########################                  conv1                  ####################################
                with tf.variable_scope('CNN1_1') as scope:
                        self.layers['CNN1_1'] = CNN_module(self.layers['relu_conv1'],'Sum',16,16)


                with tf.variable_scope('CNN1_2') as scope:
                        self.layers['CNN1_2'] = CNN_module(self.layers['CNN1_1'],'Sum',16,16)
             

                with tf.variable_scope('CNN1_3') as scope:
                        self.layers['CNN1_3'] = CNN_module(self.layers['CNN1_2'],'Concatnate',16,16)




                with tf.variable_scope('pool1') as scope:
                        self.layers['pool1'] = tf.nn.max_pool(self.layers['CNN1_3'],ksize=[1,2,2,1],strides=[1,2,2,1],padding='SAME')

                ##########################                  pool 1               ######################################


                with tf.variable_scope('CNN2_1') as scope:
                        self.layers['CNN2_1'] = CNN_module(self.layers['pool1'],'Sum',32,32)

                with tf.variable_scope('CNN2_2') as scope:
                        self.layers['CNN2_2'] = CNN_module(self.layers['CNN2_1'],'Sum',32,32)

                with tf.variable_scope('CNN2_3') as scope:
                        self.layers['CNN2_3'] = CNN_module(self.layers['CNN2_2'],'Concatnate',32,32)
                ...
                with tf.variable_scope('pool2') as scope:
                        self.layers['pool2'] = tf.nn.max_pool(self.layers['CNN2_3'],ksize=[1,2,2,1],strides=[1,2,2,1],padding='SAME')

                ##########################                  pool 2               ######################################

 
                with tf.variable_scope('CNN3_1') as scope:
                        self.layers['CNN3_1'] = CNN_module(self.layers['pool2'],'Sum',64,64)


                with tf.variable_scope('CNN3_2') as scope:
                        self.layers['CNN3_2'] = CNN_module(self.layers['CNN3_1'],'Sum',64,64)


                with tf.variable_scope('CNN3_3') as scope:
                        self.layers['CNN3_3'] = CNN_module(self.layers['CNN3_2'],'Concatnate',64,64)
               ...
               #########################################################################################################
                ...
                with tf.variable_scope('pool3') as scope:
                        self.layers['pool3'] = tf.nn.avg_pool(self.layers['CNN3_3'],ksize=[1,8,8,1],strides=[1,8,8,1],padding='SAME')


                with tf.variable_scope('softmax') as scope:
                        self.paras['W_out'] = weight_variable('W_out',[128,10])
                        self.paras['b_out'] = bias_variable('b_out',[10])
                        self.layers['output'] = full_connected(tf.reshape(self.layers['pool3'],[-1,128]),self.paras['W_out'],self.paras['b_out'],activation ='Softmax')
	...
	def run_train_epoch(self):
		...
		train_loss = 0
		self.train_epoch += 1
                print "epoch number :" ,self.train_epoch
		...
		self.train_loss.append(train_loss)
                self.train_log.write(str(train_loss)+'\n')
	...
	def run_validation(self):
		...
		self.validation_loss.append(valid_loss)
                self.validation_log.write(str(valid_loss)+'\n')
	...
	def use_optimal_paras(self):
        ...
        def run_test(self, use_optimal_parameter=True):
                ...
                average_loss = 0
                test_accuracy = 0
                batch_number = int(test_size/BATCH_SIZE)
                for i in range(batch_number):
                  x_batch = self.x_test[i*BATCH_SIZE:(i+1)*BATCH_SIZE,:]
                  y_batch = self.y_test[i*BATCH_SIZE:(i+1)*BATCH_SIZE,:]
                  ...
                  test_out = self.batch_out(feed_dict)
                  accuracy = classification_accuracy(test_out, y_batch)

                  sys.stdout.write("\r test loss : "+str(average_loss)+" | "+str(i+1)+"th/"+str(batch_number)+"batches")
                  sys.stdout.flush()
                ...
                self.test_accuracy = test_accuracy/batch_number
                self.test_loss = test_loss/batch_number
		...
		for i in range(EPOCH):
			...
			print "\n"
                        if( (i+1)%500 == 0):
                          ...
                          save_path = saver.save(self.sess, "LRNN"+str(i+1)+".ckpt")
                          print("Model saved in file: %s" % save_path)

#                saver = tf.train.Saver()
#                ckpt_path = saver.restore(self.sess,"LRNN5000.ckpt")
		self.run_test(False)
		np.save('test_loss.npy',self.test_loss)
		np.save('test_accuracy.npy',self.test_accuracy)

		saver = tf.train.Saver()
		save_path = saver.save(self.sess, "Baseline_LRNN.ckpt")
		print("Model saved in file: %s" % save_path)
